 ...
 *
 * A single MeshPhysicalMaterial (full PBR + env reflections + clearcoat + shadows) patched via
 ...
 *   - High clearcoat creates "internal depth" embedding pathways beneath the cortex shell
 *
...
const VERTEX_BODY = /* glsl */ `
  ...
  // AAA Polish: Hover adds a gentle 35% swell instead of a harsh 60% flash
  vActiveMix = clamp(uActive[cortexRegion] + uHover[cortexRegion] * 0.35, 0.0, 1.0);
...
const FRAGMENT_DIFFUSE = /* glsl */ `
  ...
  // AAA Polish: Diffuse tint is more subtle so the routing feels embedded, not painted
  diffuseColor.rgb = mix(diffuseColor.rgb, cortexCopper, clamp(cortexLine * 0.45, 0.0, 0.45));
...
const FRAGMENT_ROUGHNESS = /* glsl */ `
  ...
  roughnessFactor = clamp(roughnessFactor + (cortexRough - 0.5) * 0.14 + cortexWeave * 0.03, 0.1, 1.0);
...
export function createBrainMaterial(): BrainMaterialBundle {
  ...
  const material = new MeshPhysicalMaterial({
    ...
    roughness: 0.70,
    clearcoat: 1.0, // AAA Polish: Full clearcoat to embed circuitry
    clearcoatRoughness: 0.1, // Smooth glossy shell
