...
export const useCortex = create<CortexState>((set, get) => ({
  ...
  clearBackground: () =>
    set((s) => {
      // If inspecting, click background exits inspection but keeps selection
      if (s.focusRegion != null) return { focusRegion: null }
      // If just selected, click background clears selection
      return { selected: [], focusRegion: null }
    }),
